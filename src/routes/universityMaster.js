...
// 학사편입 경쟁률 자료 조회 (참고/확인용). university_master와는 별개 테이블이라 별도 경로로 둔다.
router.get('/academic-competition-ratio', async (req, res) => {
  const { year, univ_name, dept_name } = req.query;
  const conditions = [];
  const params = [];
  let sql = 'SELECT * FROM university_academic_competition';
  if (year) { params.push(Number(year)); conditions.push(`year = $${params.length}`); }
  if (univ_name) { params.push(`%${univ_name}%`); conditions.push(`univ_name ILIKE $${params.length}`); }
  if (dept_name) { params.push(`%${dept_name}%`); conditions.push(`dept_name ILIKE $${params.length}`); }
  if (conditions.length) sql += ' WHERE ' + conditions.join(' AND ');
...
// 학사편입 경쟁률 업로드 (예: "26학년도 학사 경쟁률.xlsx"). 기존 university_master(일반) 데이터와는
// 완전히 별도 테이블이라 일반 데이터는 전혀 건드리지 않는다. 같은 학과라도 전형별로 여러 줄이 있을 수
// 있어 upsert가 아니라, 해당 연도 기존 행을 지우고 새로 전부 넣는 방식(전체 교체)으로 처리한다.
router.post('/academic-competition-ratio-upload', requireAdmin, upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: '파일이 없습니다.' });
  const year = Number(req.body.year);
  if (!year) return res.status(400).json({ error: 'year(입시 연도)가 필요합니다.' });
  ...
  try {
    const wb = readWorkbookRobust(req.file.buffer);
    const result = parseAcademicCompetitionWorkbook(wb);
    records = result.records;
    warnings = result.warnings;
  } catch (err) {
    return res.status(422).json({ error: '학사 경쟁률 자료 파싱 실패: ' + err.message });
  }
  ...
  try {
    await client.query('BEGIN');
    await client.query('DELETE FROM university_academic_competition WHERE year=$1', [year]);
    for (let i = 0; i < records.length; i += BATCH_SIZE) {
      const batch = records.slice(i, i + BATCH_SIZE);
      const values = [];
      const placeholders = batch.map((r, idx) => {
        const base = idx * 8;
        values.push(r.univName, r.deptName, year, r.quotaAcademic, r.applicantsAcademic, r.track, r.college, r.isCombinedSelection);
        return `($${base + 1},$${base + 2},$${base + 3},$${base + 4},$${base + 5},$${base + 6},$${base + 7},$${base + 8})`;
      }).join(',');
      await client.query(
